...
func (self *Linear_struct) Forward(x [][]float64,pass *ForwardPass) [][]float64 {
  self.recent_input = x
  out,err := cutils.MatMul(&x,&self.Weight)
  if err !=nil {panic(err)}
  recent_output := cutils.VectorMatrixSum(out,self.Bias)
...
func (self *Linear_struct) Backward(Gradients *[][]float64,pass *ForwardPass) {
  ...
  if self.index == num_layers {
    ...
    previous_weight := cutils.Transpose((*pass).Layers[self.index+1].Weight)
    
    self.Delta_Weight,_ = cutils.MatMul(&previous_delta,&previous_weight)
  }
}
...
func (self MomentumDescent) Step(Gradients *[][]float64) {
  ...
  for i := num_layers-1; i >=0; i-- {
    ...
    transposed_recent_input := cutils.Transpose(current_layer.recent_input)
    Weight_grads,_ := cutils.MatMul(&transposed_recent_input,&current_layer.Delta_Weight)
    ...
    w_grads_transposed := cutils.Transpose(Delta)
    for i := range shape[1] {
    ...
    Bias_grads = cutils.Transpose(Bias_grads)
