...
__global__ void AddKernel(double* out, const double* a, const double* b, int size) {
...
void AddWrapper(double* out, const double* a, const double* b, int size) {
    ...
    AddKernel<<<blocksPerGrid, threadsPerBlock>>>(d_out, d_a, d_b, size);
